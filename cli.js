...
function calculateTokenSavings(content, projectPath) {
	...
	const savingsPercent = fullTokenCount > 0 ? ((tokensSaved / fullTokenCount) * 100).toFixed(1) : 0;
...
function saveToFile(content, filePath, projectPath) {
	...
	fs.appendFileSync(
		fullPath,
		`\n\n<!-- Token estimate: ${tokenCount} (saved ~${tokensSaved} tokens, ${savingsPercent}%) -->`,
		'utf8'
	);
}
...
async function main() {
	...
	try {
		...
		if (argv.output) {
			...
			const { tokenCount, tokensSaved, savingsPercent } = calculateTokenSavings(
				prompt,
				projectPath
			);
